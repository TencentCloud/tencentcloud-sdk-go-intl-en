...
func (r *DescribeMediaLiveHighlightResultRequest) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *DescribeMediaLiveHighlightResultRequest) FromJsonString(s string) error {
	f := make(map[string]interface{})
	if err := json.Unmarshal([]byte(s), &f); err != nil {
		return err
	}
	...
	delete(f, "PageSize")
	if len(f) > 0 {
		return tcerr.NewTencentCloudSDKError("ClientError.BuildRequestError", "DescribeMediaLiveHighlightResultRequest has unknown keys!", "")
	}
	return json.Unmarshal([]byte(s), &r)
}
...
func (r *DescribeMediaLiveHighlightResultResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *DescribeMediaLiveHighlightResultResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
