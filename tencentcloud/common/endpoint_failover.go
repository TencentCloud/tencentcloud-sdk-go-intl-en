...
// isResponseHealthy classifies a response as healthy based solely on the
// transport error and HTTP status code. Body content is not inspected —
// business-level errors (HTTP 200 + JSON error body) are handled by the
// response parser, not by the failover breaker.
