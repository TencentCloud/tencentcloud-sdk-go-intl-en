...
// Domain families (N)
//
//	N1: Normal — tencentcloudapi.{com,com.cn,cn}
//	N2: AI — ai.tencentcloudapi.{com,com.cn,cn}
//	N3: Internal — internal.tencentcloudapi.{com,com.cn,cn}
//
// Test scenarios (M)
//
// Pure helpers
...
//	15. 200 + invalid JSON body — no failover (body not inspected)
//	16. 200 + empty body — no failover (body not inspected)
//
...
var (
	n1 = family{
		...
		firstFailover: "cvm.tencentcloudapi.com.cn", secondFailover: "cvm.tencentcloudapi.cn",
		allTldHosts: []string{"cvm.tencentcloudapi.com", "cvm.tencentcloudapi.com.cn", "cvm.tencentcloudapi.cn"},
	}
	n2 = family{
		name: "AI", originHost: "cvm.ai.tencentcloudapi.com",
		firstFailover: "cvm.ai.tencentcloudapi.com.cn", secondFailover: "cvm.ai.tencentcloudapi.cn",
		allTldHosts: []string{"cvm.ai.tencentcloudapi.com", "cvm.ai.tencentcloudapi.com.cn", "cvm.ai.tencentcloudapi.cn"},
	}
	n3 = family{
		name: "Internal", originHost: "cvm.internal.tencentcloudapi.com",
		firstFailover: "cvm.internal.tencentcloudapi.com.cn", secondFailover: "cvm.internal.tencentcloudapi.cn",
		allTldHosts: []string{"cvm.internal.tencentcloudapi.com", "cvm.internal.tencentcloudapi.com.cn", "cvm.internal.tencentcloudapi.cn"},
	}
...
func (s *transportStub) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	s.requestsSeen = append(s.requestsSeen, req)
	if req.Body != nil {
		b, _ := ioutil.ReadAll(req.Body)
		req.Body.Close()
		s.bodiesSeen = append(s.bodiesSeen, string(b))
	} else {
		s.bodiesSeen = append(s.bodiesSeen, "")
	}
...
func newReq(host string) *http.Request {
	...
	// selectHost now derives the service prefix from the host's first label,
	// so rb.service is no longer consulted; rb itself is still stashed to
	// mirror what Client.Send builds in production (resignForHost needs it).
	rb := &requestBuilder{service: defaultEndpointFailover.serviceOf(host)}
	return req.WithContext(withRequestBuilder(req.Context(), rb))
}

// selectHostForTest wraps selectHost to pull the requestBuilder from the
// request context — matching how send() calls it in production.
func selectHostForTest(req *http.Request, c *Client) candidate {
	return defaultEndpointFailover.selectHost(req, c)
}
...
func Test_hostWithTldBuildsCorrectHosts(t *testing.T) {
	...
	cand := selectHostForTest(newReq("cvm.tencentcloudapi.com"), c)
...
func Test_hostWithTldFromCnAndComCnOrigins(t *testing.T) {
	...
	cand := selectHostForTest(newReq("cvm.tencentcloudapi.cn"), c)
	if cand.host != "cvm.tencentcloudapi.com" {
		t.Fatalf("cn origin: got %v, want .com", cand)
	}

	resetBreakers()
	c.profile.HttpProfile.Endpoint = "cvm.tencentcloudapi.com.cn"
	tripBreakerFor("cvm.tencentcloudapi.com.cn", time.Hour)
	cand = selectHostForTest(newReq("cvm.tencentcloudapi.com.cn"), c)
	if cand.host != "cvm.tencentcloudapi.cn" {
		t.Fatalf("com.cn origin: got %v, want .cn", cand)
	}
}

func Test_hostWithTldDropsRegionFromPrefix(t *testing.T) {
	c := newClient(n1)

	tripBreakerFor("cvm.ap-guangzhou.tencentcloudapi.com", time.Hour)
	cand := selectHostForTest(newReq("cvm.ap-guangzhou.tencentcloudapi.com"), c)
	// failover target is service + failover TLD; region label is dropped.
	if cand.host != "cvm.tencentcloudapi.com.cn" {
		t.Fatalf("got %v, want cvm.tencentcloudapi.com.cn", cand)
	}
}

func Test_hostWithTldDropsIntlPrefix(t *testing.T) {
	c := newClient(n1)

	tripBreakerFor("cvm.intl.tencentcloudapi.com", time.Hour)
	cand := selectHostForTest(newReq("cvm.intl.tencentcloudapi.com"), c)
	if cand.host != "cvm.tencentcloudapi.com.cn" {
		t.Fatalf("first failover: got %v, want cvm.tencentcloudapi.com.cn", cand)
	}

	tripBreakerFor("cvm.tencentcloudapi.com.cn", time.Hour)
	cand = selectHostForTest(newReq("cvm.intl.tencentcloudapi.com"), c)
	if cand.host != "cvm.tencentcloudapi.cn" {
		t.Fatalf("second failover: got %v, want cvm.tencentcloudapi.cn", cand)
	}
}
...
// M15: 200 + invalid JSON body — failover no longer inspects body content,
// so isResponseHealthy returns true (breaker records success). The parse
// layer still surfaces the error to the caller, but no failover is triggered.
func Test_invalidJsonBodyDoesNotTriggerFailover(t *testing.T) {
	for _, f := range families {
		c := newClient(f)
		s := installStub(c)
		...
		if len(s.hostsSeen) != 1 {
			t.Fatalf("%s: expected 1 attempt (no failover), got %d", f.name, len(s.hostsSeen))
		}
		if s.hostsSeen[0] != f.originHost {
			t.Fatalf("%s: host = %s, want %s", f.name, s.hostsSeen[0], f.originHost)
		}
...
// M16: 200 + empty body — same as M15: healthy at the transport layer,
// parse-layer error surfaces to caller, no failover.
func Test_emptyBodyDoesNotTriggerFailover(t *testing.T) {
	for _, f := range families {
		c := newClient(f)
		s := installStub(c)
...
func Test_regionPinnedHostFailoverDropsRegionPrefix(t *testing.T) {
	cpf := profile.NewClientProfile()
	cpf.DisableRegionBreaker = false
	...
	if s.hostsSeen[0] != "cvm.tencentcloudapi.com.cn" {
		t.Fatalf("host = %s, want cvm.tencentcloudapi.com.cn (region dropped)", s.hostsSeen[0])
	}
}
...
// Test_sseBodyIsNotConsumedByFailover verifies that response bodies are not
// read/buffered by isResponseHealthy, so the stream remains intact for the
// caller. With body inspection removed, this holds for all content types —
// SSE is used here as the canonical streaming case.
...
func Test_regionPinnedHostIsEligibleForFailover(t *testing.T) {
	regionHosts := []struct {
		...
		{name: "Normal-R", host: "cvm.ap-guangzhou.tencentcloudapi.com"},
		{name: "AI-R", host: "hunyuan.ap-guangzhou.ai.tencentcloudapi.com"},
		{name: "Internal-R", host: "cvm.ap-guangzhou.internal.tencentcloudapi.com"},
	}
	for _, f := range regionHosts {
		c := newClient(family{name: f.name, originHost: f.host})
		s := installStub(c)
		...
		if s.hostsSeen[0] != f.host {
			t.Fatalf("%s: host = %s, want %s", f.name, s.hostsSeen[0], f.host)
		}
	}
}
...
func Test_failoverRebuildPreservesSkipSignWithoutCredential(t *testing.T) {
	resetBreakers()
	cpf := profile.NewClientProfile()
	cpf.DisableRegionBreaker = false
	cpf.HttpProfile.Endpoint = n1.originHost
	c := NewCommonClient(nil, regions.Guangzhou, cpf)
	s := installStub(c)
	tripBreakerFor(n1.originHost, time.Hour)
	s.programOk()

	req := tchttp.NewCommonRequest("cvm", "2017-03-12", "DescribeInstances")
	req.SetSkipSign(true)
	if err := c.Send(req, tchttp.NewCommonResponse()); err != nil {
		t.Fatalf("expected success: %v", err)
	}
	if len(s.hostsSeen) != 1 || s.hostsSeen[0] != n1.firstFailover {
		t.Fatalf("hostsSeen = %v, want [%s]", s.hostsSeen, n1.firstFailover)
	}
	if len(s.authsSeen) != 1 || s.authsSeen[0] != "SKIP" {
		t.Fatalf("authsSeen = %v, want [SKIP]", s.authsSeen)
	}
}

func Test_failoverRebuildPreservesContext(t *testing.T) {
	type ctxKey struct{}
	c := newClient(n1)
	s := installStub(c)
	tripBreakerFor(n1.originHost, time.Hour)
	...
	req := newCvmRequest()
	req.SetContext(context.WithValue(context.Background(), ctxKey{}, "marker"))
	if err := c.Send(req, tchttp.NewCommonResponse()); err != nil {
		t.Fatalf("expected success: %v", err)
	}
	if len(s.requestsSeen) != 1 {
		t.Fatalf("expected 1 request, got %d", len(s.requestsSeen))
	}
	if got := s.requestsSeen[0].Context().Value(ctxKey{}); got != "marker" {
		t.Fatalf("context value = %v, want marker", got)
	}
}

func Test_failoverRebuildPreservesUnsignedPayload(t *testing.T) {
	resetBreakers()
	cpf := profile.NewClientProfile()
	cpf.DisableRegionBreaker = false
	cpf.UnsignedPayload = true
	cpf.HttpProfile.Endpoint = n1.originHost
	c := NewCommonClient(NewCredential("AKIDTEST", "SKTEST"), regions.Guangzhou, cpf)
	s := installStub(c)
	tripBreakerFor(n1.originHost, time.Hour)
	...
	if got := s.requestsSeen[0].Header.Get("X-TC-Content-SHA256"); got != "UNSIGNED-PAYLOAD" {
		t.Fatalf("X-TC-Content-SHA256 = %q, want UNSIGNED-PAYLOAD", got)
	}
}

// Even when the user sets scheme=http, a failover request must be sent over
// https — failover endpoints are https-only.
func Test_failoverRebuildForcesHttps(t *testing.T) {
	resetBreakers()
	cpf := profile.NewClientProfile()
	cpf.DisableRegionBreaker = false
	cpf.HttpProfile.Endpoint = n1.originHost
	...
	if got := s.requestsSeen[0].URL.Scheme; got != "https" {
		t.Fatalf("URL.Scheme = %q, want https", got)
	}
	// Sanity: failover host was actually used (not the tripped origin).
	if got := s.requestsSeen[0].URL.Hostname(); got != "cvm.tencentcloudapi.com.cn" {
		t.Fatalf("URL.Host = %s, want cvm.tencentcloudapi.com.cn", got)
	}
}
